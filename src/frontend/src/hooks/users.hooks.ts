...
import { User, AuthenticatedUser, UserSettings, UserRole, Project } from 'shared';
...
export const useUpdateUserRole = () => {
  const auth = useAuth();
  const queryClient = useQueryClient();
  return useMutation<{ message: string }, Error, UserRole>(
    ['users', 'change-role'],
    async (updateUserPayload: UserRole) => {
