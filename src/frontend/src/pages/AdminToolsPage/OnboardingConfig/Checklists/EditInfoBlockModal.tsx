...
import { useEditChecklist } from '../../../../hooks/onboarding.hook';
...
const EditInfoBlockModal: React.FC<EditInfoBlockModalProps> = ({ open, handleClose, parentChecklist, defaultValues }) => {
  const theme = useTheme();
  const toast = useToast();
  const { mutateAsync: editChecklist } = useEditChecklist(defaultValues.checklistId);
  ...
  const onFormSubmit = async (data: InfoBlockFormValues) => {
    try {
      ...
      toast.error(error.message || 'Failed to update information block');
    }
  };
