...
export const convertToBarTasks = (
  ...
  let barTasks = tasks.map((t, i) => {
    return convertToBarTask(
      ...
      handleWidth,
      rtl,
      t.styles?.backgroundColor ?? barProgressColor,
      t.styles?.backgroundColor ?? barProgressSelectedColor,
      barBackgroundColor,
      barBackgroundSelectedColor,
      t.styles?.backgroundColor ?? projectProgressColor,
      t.styles?.backgroundColor ?? projectProgressSelectedColor,
...
const taskXCoordinate = (xDate: Date, dates: Date[], columnWidth: number) => {
  ...
  let x;
  if (dates[index]) {
    const remainderMillis = dates[index] ? xDate.getTime() - dates[index].getTime() : xDate.getTime();
    const percentOfInterval = remainderMillis / (dates[index + 1].getTime() - dates[index].getTime());
    x = index * columnWidth + percentOfInterval * columnWidth;
  } else {
    x = index * columnWidth + columnWidth;
  }
  return x;
};
