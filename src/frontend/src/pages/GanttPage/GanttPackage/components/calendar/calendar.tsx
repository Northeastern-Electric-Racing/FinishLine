...
export const Calendar = ({
  ...
  const getCalendarValuesForYear = () => {
    ...
    for (let i = 0; i < dateSetup.dates.length; i++) {
      ...
      bottomValues.push(
        <text key={date.getMonth()} y={headerHeight * 0.8} x={columnWidth} className={styles.calendarBottomText}>
          {bottomValue}
        </text>
      );
  ...
  const getCalendarValuesForWeek = () => {
    ...
    for (let i = dates.length - 1; i >= 0; i--) {
      ...
      bottomValues.push(
        <text key={date.getTime()} y={headerHeight * 0.8} x={columnWidth * (i + +rtl)} className={styles.calendarBottomText}>
          {bottomValue}
        </text>
      );
