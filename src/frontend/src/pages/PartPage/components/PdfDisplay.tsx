...
import { Part_Review_Popup, PartReview } from 'shared';
...
const PDFViewer: React.FC<FileDisplayProps> = ({ review, reviewMode }) => {
  const [scale, setScale] = useState<number>(1);
  const [position, setPosition] = useState({ x: 0, y: 0 });
  const [isDragging, setIsDragging] = useState(false);
  const [startPos, setStartPos] = useState({ x: 0, y: 0 });
  const [variablePopups, setVariablePopups] = useState<Part_Review_Popup[]>([...review.popUps]);
