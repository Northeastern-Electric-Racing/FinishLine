...
const performSeed: () => Promise<void> = async () => {
  ...
  */
  ...
  // create ruleset types
  const fsaeRulesetType = await prisma.ruleset_Type.create({
    data: ruleSeedData.rulesetTypeFSAE(batman.userId, ner.organizationId)
  });

  const fheRulesetType = await prisma.ruleset_Type.create({
    data: ruleSeedData.rulesetTypeFHE(batman.userId, ner.organizationId)
  });

  const mockRulesetType = await prisma.ruleset_Type.create({
    data: ruleSeedData.mockRulesetType(batman.userId, ner.organizationId)
  });
  ...
  const rulesetMock = await prisma.ruleset.create({
    data: ruleSeedData.rulesetMock(fergus.carId, batman.userId, mockRulesetType.rulesetTypeId)
  });

  // seed the rulesets, and add rules to bodywork project
  await seedFsaeRules(
    prisma,
