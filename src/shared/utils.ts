...
export const isWithinBuffer = (scheduledDate: Date, newDate: Date, bufferDays: number) => {
  const UTCScheduledDate = Date.UTC(scheduledDate.getFullYear(), scheduledDate.getMonth(), scheduledDate.getDay());
  const UTCNewDate = Date.UTC(newDate.getFullYear(), newDate.getMonth(), newDate.getDay());
